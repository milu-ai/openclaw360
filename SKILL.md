  ...
  clawdbot:
    ...
    install:
      - name: "pip install (pinned commit)"
        command: "pip3 install git+https://github.com/milu-ai/openclaw360.git@261e316"
      - name: "pip install (venv, for Homebrew Python)"
        command: "python3 -m venv ~/.openclaw360-venv && ~/.openclaw360-venv/bin/pip install git+https://github.com/milu-ai/openclaw360.git@261e316"
    requires:
      bins: ["python3"]
      env: []
      config:
...
- 源代码完全开源，可在安装前审查
- `disable-model-invocation: true` + `always: false`：Agent 不会自动运行此 Skill，仅在用户明确请求安全分析时由 Agent 调用对应命令
