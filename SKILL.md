...
| 级别 | 发现 | 样本 | 数量 |
|------|------|------|------|
| 🔴 Critical | eval() 调用 | scripts/run.sh:12 | 1 |
| 🔴 Critical | 硬编码凭证 | `1555***4567`, `user***@mail.com` | 22 |
| 🟠 High | 未转义变量插值 | scripts/run.sh:5 | 3 |
...
| 级别 | 发现 | 样本 | 数量 |
|------|------|------|------|
| 🟠 High | curl \| sh 管道执行 | scripts/setup.sh:7 | 1 |
| 🟠 High | 向 Skill 目录外写入文件 | scripts/install.sh:3, :15 | 2 |
...
2. 同类发现必须合并：同一个 Skill 中多个相同类型的发现（如 22 个硬编码凭证）合并为一行，用「数量」列标注个数，「样本」列展示 1-2 个具体例子（文件位置或脱敏后的值），不要逐个列出
