...
- **安装时**：需要访问 `github.com` 下载源代码（仅一次）
- **运行时**：所有安全检测（prompt/tool/output/scan）完全离线，不发起任何网络请求
- **规则更新**（`openclaw360 update`）：需要网络访问，但必须由用户手动触发，不会自动执行
- 除上述两种情况外，本 Skill 不发起任何网络连接
...
```bash
pip3 install git+https://github.com/milu-ai/openclaw360.git@v0.1.0
```
...
~/.openclaw360-venv/bin/pip install git+https://github.com/milu-ai/openclaw360.git@v0.1.0
```
